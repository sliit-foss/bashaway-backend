...
export const retrieve = async (event_id, user) => {
  ...
  if (!result) throw new createError(404, "Event doesn't exist or you do not have permission to view this event");
  return result;
};

export const update = async (event_id, data, user) => {
  const event = await eventRepository.findById(event_id, user);
  if (!event) throw new createError(404, "Event doesn't exist to update");
  if (data.name) {
...
export const deleteOne = async (event_id, user) => {
  const event = await eventRepository.findById(event_id, user);
  if (!event) throw new createError(400, "Event doesn't exist to remove");
