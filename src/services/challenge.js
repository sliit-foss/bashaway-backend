...
export const update = async (challenge_id, data, user) => {
  ...
  if (!challenge) throw new createError(404, "Challenge doesn't exist to update");
  if (data.name) {
    const check = await challengeRepository.findOne({ name: data.name, _id: { $ne: challenge_id } });
    if (check) throw new createError(400, 'Challenge name already taken');
  }
  ...
  if (data.max_score && data.max_score !== challenge.max_score) {
    const submissionCount = (await submissionRepository.getDistinctSubmissions(challenge_id)).length;
    if (submissionCount > 0) throw new createError(400, 'Cannot update a challenge with submissions');
  }
  return challengeRepository.findOneAndUpdate({ _id: challenge_id }, data);
};
...
export const deleteOne = async (challenge_id, user) => {
  ...
  const checkSubmission = await submissionRepository.findOne({ challenge: challenge_id });
  if (checkSubmission) {
